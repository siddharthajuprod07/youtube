...
def get_credentials(sessionKey):
    try:
        service = client.connect(app="tmdb",owner="admin",token=sessionKey)
        secrets = service.storage_passwords
        for secret in secrets:
            secret_content = secret.content
            logger.info(secret_content)
            api_key = secret_content["clear_password"]
...
def main():
    ...
    api_key = get_credentials(sessionKey)
