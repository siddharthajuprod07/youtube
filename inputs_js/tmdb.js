require([
    ...
    defaultTokenModel.set("release_year_token_val", "*");
	...
	multi1.on("change",function(){
        ...
        modified_val = multi1.val()
        //console.log(modified_val)
        transformed_release_yr=""
        $.each(modified_val, function(index, item) {
            //console.log(item)
            transformed_release_yr = transformed_release_yr + 'release_year="' + item + '" OR '
        });
        //console.log(transformed_release_yr)
        release_year_token_val = transformed_release_yr.substr(0,transformed_release_yr.lastIndexOf("OR")-1)
        console.log(release_year_token_val)
        
        defaultTokenModel.set("release_year_token_val", release_year_token_val);
	}
	)
	...
	var single_pnl_post_srch = new PostProcessManager({
		...
		managerid: "base_search",
		search: "| search $release_year_token_val$ genre_id=$genre_id$ original_language=$orig_ln$ | dedup id | stats count" 
	}, {tokens: true});
	...
	var pie_chart_post_srch = new PostProcessManager({
		id: "pie_chart_srch",
		managerid: "base_search",
		search: mvc.tokenSafe("| search $release_year_token_val$ genre_id=$genre_id$ original_language=$orig_ln$ | dedup id | stats count by release_year") 
	});
	...
	var bar_chart_post_srch = new PostProcessManager({
		id: "bar_chart_srch",
		managerid: "base_search",
		search: mvc.tokenSafe("| search $release_year_token_val$ genre_id=$genre_id$ original_language=$orig_ln$ | dedup id | stats count by original_language") 
	});
	...
	var line_chart_post_srch = new PostProcessManager({
		id: "line_chart_srch",
		managerid: "base_search",
		search: mvc.tokenSafe("| search $release_year_token_val$ genre_id=$genre_id$ original_language=$orig_ln$ | dedup id | stats count by genre_id") 
	});
    ...
    var movie_dtl_srch = new PostProcessManager({
		id: "tbl_srch",
		managerid: "base_search",
		search: "| search $release_year_token_val$ genre_id=$genre_id$ original_language=$orig_ln$ | dedup id | table release_date,title,popularity,backdrop_path,overview | where backdrop_path != \"null\"" 
	}, {tokens: true});
